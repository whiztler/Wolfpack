if (ADF_HC_execute && !WP_convoyStarted) then {[180] execVM "Scr\convoy.sqf"}; // in case the RV has not been triggered.

if (hasInterface) then {
	if !(isNil "SOD_1_SCC") then {
		hintSilent parseText format ["<t color='#6C7169' align='left'>Good morning Commander %1, good to see you again.</t>",name vehicle SOD_1_SCC];
	} else {
		hintSilent parseText"<t color='#6C7169' align='left'>Good morning Commander, good to see you again.</t>";
	};
